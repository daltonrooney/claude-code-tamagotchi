  ...
  spawnAnalysisWorker(transcriptPath: string, sessionId: string, petState?: any): void {
    ...
    const worker = spawn('bun', [
      ...
      env: {
        ...
        PET_AI_API_KEY: this.config.aiApiKey,
        PET_AI_BASE_URL: this.config.aiBaseUrl,
        PET_AI_MODEL: this.config.aiModel,
        PET_AI_TIMEOUT: String(this.config.aiTimeout),
        PET_AI_MAX_RETRIES: String(this.config.aiMaxRetries),
        // Legacy Groq configuration (for backward compatibility)
