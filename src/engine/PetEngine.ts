...
export class PetEngine {
  ...
  private processAction(action: PetAction): void {
    ...
    switch (action.command) {
      ...
      case 'drink':
        this.handleDrink(action.parameter || 'water');
        break;
    }
  }
  ...
  private handleDrink(drink: string): void {
    if (!this.state) return;

    const sillyDrinks = ['beer', 'wine'];
    const isSilly = sillyDrinks.includes(drink.toLowerCase());
    ...
    this.state.pendingAction = {
      ...
      item: drink,
      startTime: Date.now(),
      duration: isSilly ? 12 : 5, // Silly drinks last longer
      updateCount: 0
    };

    // Set mood to silly if it's an alcoholic drink
    if (isSilly) {
      this.state.currentMood = 'silly';
      this.state.systemMessage = `*hic* ${this.state.name} is feeling a bit silly! 🥴`;
    } else {
      this.state.systemMessage = `Ahh, refreshing ${drink}! 💧`;
    }
    this.state.messageTimestamp = Date.now();

    // Slight energy boost from hydration
