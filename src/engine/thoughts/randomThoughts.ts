...
export class RandomThoughts {
  ...
  // Get silly drunk thoughts (after beer/wine)
  static getSillyDrunkThought(state: PetState): string {
    const sillyDrunkThoughts = [
      "*hic* I love you, you know that? 🥴",
      "This code looks AMAZING... wait is that a bug?",
      "*hic* We should refactor EVERYTHING!",
      "You're my best friend... *hic* ...and so is this keyboard",
      "I have the BEST idea for a startup...",
      "Why is the room spinning? Oh wait, it's the loading spinner",
      "*hic* Let's deploy to production! What could go wrong?",
      "I'm not drunk, I'm just... *hic* ...optimized!",
      "Everything is funnier when you're silly 🍺",
      "This bug isn't that bad actually... *hic* ...it's GREAT!",
      "*hic* Hey... hey... semicolons are just fancy commas",
      "I can definitely fix this in 5 minutes... *hic*",
      "You know what? Comments are overrated anyway",
      "*hic* Let's add more dependencies! More is better!",
      "I'm seeing double... double the productivity! 🥴",
      "The code is blurry but my vision is CLEAR",
      "*hic* Tabs AND spaces... why not both?",
      "Is it hot in here or is it just the... *hic* ...the beer?",
      "We should totally mass email the CEO right now",
      "*hic* Who needs tests when you have confidence?!"
    ];

    return sillyDrunkThoughts[Math.floor(Math.random() * sillyDrunkThoughts.length)];
  }
