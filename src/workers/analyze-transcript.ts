...
import { LocalAIClient } from '../llm/LocalAIClient';
import { 
  TranscriptMessage, 
  MessageMetadata, 
  Feedback,
...
// Configuration from environment (prioritize new AI config, fallback to Groq for backward compatibility)
const config = {
  aiApiKey: process.env.PET_AI_API_KEY || process.env.PET_GROQ_API_KEY || 'dummy-key',
  ...
  aiMaxRetries: parseInt(process.env.PET_AI_MAX_RETRIES || '2'),
...
Output preview: ${userContent.slice(0, 500)}`;
      ...
      const analysis = await aiClient.analyzeUserMessage(
        toolResultPrompt,
        sessionHistory
      );
  ...
  const analysis = await aiClient.analyzeExchange(
