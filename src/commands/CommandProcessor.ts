...
// Define valid drinks
const VALID_DRINKS: Record<string, { emoji: string; effect?: string }> = {
  'water': { emoji: '💧' },
  'coffee': { emoji: '☕' },
  'tea': { emoji: '🍵' },
  'juice': { emoji: '🧃' },
  'soda': { emoji: '🥤' },
  'beer': { emoji: '🍺', effect: 'silly' },
  'wine': { emoji: '🍷', effect: 'silly' },
  'milkshake': { emoji: '🥛' }
};
...
export class CommandProcessor {
  ...
  static async processCommand(input: string): Promise<string> {
    ...
    switch (command) {
      ...
      case 'drink':
        const drink = parameter || 'water';

        // Validate drink
        if (!VALID_DRINKS[drink.toLowerCase()]) {
          return `Sorry, "${drink}" is not a valid drink. Try: ${Object.keys(VALID_DRINKS).join(', ')}`;
        }

        const drinkInfo = VALID_DRINKS[drink.toLowerCase()];
        await this.writeAction('drink', drink.toLowerCase());

        if (drinkInfo.effect === 'silly') {
          return `${state?.name || 'Your pet'} drinks the ${drink}... *hic* 🥴 ${drinkInfo.emoji}`;
        }
        return `Giving ${drink} to your pet! ${drinkInfo.emoji}`;
...
🍼 CARE: /pet-pet, /pet-feed [food], /pet-play [toy], /pet-drink [drink], /pet-clean, /pet-sleep
📊 INFO: /pet-status, /pet-stats, /pet-help
🎨 CUSTOM: /pet-name [name]
⚙️ MANAGE: /pet-reset

🍽️ FOODS: cookie, pizza, sushi, apple, carrot, steak, fish, candy, takeout, mango, prawn, pretzel, bagel
🎮 TOYS: ball, frisbee, laser, yarn, puzzle
🍹 DRINKS: water, coffee, tea, juice, soda, beer, wine, milkshake
