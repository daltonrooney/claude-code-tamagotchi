...
export interface Config {
  ...
  // AI Configuration (supports both Groq and local OpenAI endpoints)
  aiApiKey?: string;
  aiBaseUrl: string;
  aiModel: string;
  aiTimeout: number;
  aiMaxRetries: number;
  // Legacy Groq support (for backward compatibility)
  groqApiKey?: string;
  groqModel?: string;
  groqTimeout?: number;
...
export const config: Config = {
  ...
  // AI Configuration (prioritize new config, fallback to Groq for backward compatibility)
  aiApiKey: process.env.PET_AI_API_KEY || process.env.PET_GROQ_API_KEY || process.env.GROQ_API_KEY || 'dummy-key',
  aiBaseUrl: process.env.PET_AI_BASE_URL || 'http://127.0.0.1:11435/v1',
  aiModel: process.env.PET_AI_MODEL || 'gpt-5-nano',
  aiTimeout: parseInt(process.env.PET_AI_TIMEOUT || process.env.PET_GROQ_TIMEOUT || '2000'),
  aiMaxRetries: parseInt(process.env.PET_AI_MAX_RETRIES || process.env.PET_GROQ_MAX_RETRIES || '2'),
  // Legacy Groq support (for backward compatibility)
